...
BarWidget {
  ...
  // Fixed UTC offset in minutes for this instance ("timezoneOffsetMinutes").
  // Absent = local system time, exactly the built-in clock's behavior; any
  // number including 0 pins the clock to that UTC offset (0 = UTC/GMT, -300
  // = EST, +240 = GST — all fixed-offset zones with no daylight saving, so
  // a plain offset is exact for them).
  readonly property var zoneOffsetRaw: setting("timezoneOffsetMinutes", null)
  readonly property bool usesFixedZone: zoneOffsetRaw !== null && zoneOffsetRaw !== undefined
  readonly property int zoneOffset: usesFixedZone ? Number(zoneOffsetRaw) : 0
  // Optional label prefixed to the bar text, e.g. "EST" or "GMT".
  readonly property string zoneLabel: String(setting("label", "") || "")
  ...
  function formatted(date) {
    var text = root.usesFixedZone
      ? Model.formatZone(date, root.zoneOffset, root.activeFormat)
      : Qt.formatDateTime(date, root.activeFormat.replace(/ww/g, Model.isoWeekLiteral(date.getFullYear(), date.getMonth(), date.getDate())))
    if (root.zoneLabel !== "" && !root.vertical) text = root.zoneLabel + " " + text
    return text
  }
