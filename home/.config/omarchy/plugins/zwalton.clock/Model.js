...
// ---- Fixed-offset zone formatting (zwalton.clock customization).
//      Qt.formatDateTime only knows the system timezone, so a clock pinned
//      to another zone has to format the shifted UTC wall clock itself.
//      Supports the same Qt-style tokens as the local path: dddd/ddd/dd/d,
//      MMMM/MMM/MM/M, yyyy/yy, HH/hh/h, mm, AP/ap, '…' literals, and 'W'ww
//      for ISO week. Names are English; the locale path is untouched unless
//      an explicit offset is set. Offsets are fixed (no DST), exact for
//      zones like EST (-300), GST (+240), and GMT/UTC (0).
...
function formatZone(date, offsetMinutes, format) {
  var shifted = new Date(date.getTime() + Number(offsetMinutes) * 60000)
  var year = shifted.getUTCFullYear()
  var month = shifted.getUTCMonth()
  var day = shifted.getUTCDate()
  var weekday = shifted.getUTCDay()
  var hours = shifted.getUTCHours()
  var minutes = shifted.getUTCMinutes()
  var hour12 = hours % 12
  if (hour12 === 0) hour12 = 12

  // Longest tokens first. Substituted values go straight into `out` and are
  // never rescanned, so names like "Tuesday" or "AM" cannot be clobbered by
  // single-letter tokens.
  var tokens = [
    ["dddd", DAY_NAMES[weekday]],
    ["ddd", DAY_SHORT[weekday]],
    ["MMMM", MONTH_NAMES[month]],
    ["MMM", MONTH_SHORT[month]],
    ["yyyy", pad2(Math.floor(year / 100)) + pad2(year % 100)],
    ["HH", pad2(hours)],
    ["hh", pad2(hour12)],
    ["mm", pad2(minutes)],
    ["AP", hours < 12 ? "AM" : "PM"],
    ...
    ["dd", pad2(day)],
    ["yy", pad2(year % 100)],
    ["ww", isoWeekLiteral(year, month, day)],
    ["MM", pad2(month + 1)],
    ["h", hour12],
    ["d", day],
    ["M", month + 1]
  ...
  while (i < formatStr.length) {
    ...
    for (var t = 0; t < tokens.length; t++) {
      if (formatStr.substr(i, tokens[t][0].length) === tokens[t][0]) {
        matched = tokens[t][1]
        i += tokens[t][0].length
        break
      }
